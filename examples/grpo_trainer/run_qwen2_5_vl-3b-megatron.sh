...
HF_MODEL_PATH=$HOME/Qwen2.5-VL-7B-Instruct
DIST_CKPT_PATH=$HOME/Qwen2.5-VL-7B-Instruct-mcore

# convert HF model to verl format
#python converter_hf_to_mcore.py --hf_model_path $HF_MODEL_PATH --output_path $DIST_CKPT_PATH

#train_path=$HOME/data/gsm8k/train.parquet
#train_path=$HOME/data/geo3k/test.parquet
train_path=/nlp_group/huangjiaming/logits-distill/merged.parquet
#train_path=/nlp_group/huangjiaming/logits-distill/pure_geo3k.parquet
...
PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo --config-path=config \
    ...
    trainer.total_epochs=15 2>&1 | tee qwen2_5_vl.log
